...
public class FilterNBTContainer extends AbstractContainerMenu {
    ...
    public FilterNBTContainer(int windowId, Inventory playerInventory, Player player, ItemStack filterItem) {
        super(LaserIORegistration.FilterNBT_Container.get(), windowId);
        playerEntity = player;
    ...
    public FilterNBTContainer(int windowId, Inventory playerInventory, Player player, BlockPos sourcePos, ItemStack filterItem, ItemStack sourceCard, byte direction, int slotIndex) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index >= SLOTS) {
                for (int i = 0; i < SLOTS; i++) { //Loop through slots
                    handler.set(i, ItemResource.EMPTY, 0); //Clear the current slots
                }
    ...
    public void removed(Player playerIn) {
        ...
        if (!world.isClientSide()) {
            handler.set(0, ItemResource.EMPTY, 0); //Clear the current slot
            if (sourceCard != null && !sourceCard.isEmpty()) {
                CardItemHandler cardItemHandler = BaseCard.getInventory(sourceCard);
                cardItemHandler.set(0, ItemResource.of(filterItem), filterItem.getCount());
            }
